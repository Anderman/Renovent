import { extractMenuItems } from "./webui-utils.js";
...
export async function fetchKeyPressLogData() {
...
export async function fetchSettingsMenuData(previousCompletedMs = 0) {
	...
	const status = await poll(async () => fetchStatusData(), (result) =>
		!result.settingsMenuRunning && Number(result.settingsMenuLastCompletedMs ?? 0) !== Number(previousCompletedMs ?? 0), 20000, 500);
	return {
		status,
...
export async function fetchSensorsMenuData(previousCompletedMs = 0) {
	...
	return poll(async () => getJson("/api/sensors-menu"), (result) =>
		!result.running && Number(result.lastCompletedMs ?? 0) !== Number(previousCompletedMs ?? 0), 20000, 500);
}

export async function fetchLatestSensorsMenuData() {
...
export async function postMqttConfigData(config) {
	return getJson("/api/mqtt/config", {
		method: "POST",
		headers: { "Content-Type": "application/json" },
